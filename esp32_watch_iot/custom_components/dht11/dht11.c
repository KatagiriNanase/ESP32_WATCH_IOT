...
#define TAG		"DHT11"
#define DHT11_GET_PERIOD 10*1000

uint8_t DHT11_PIN = -1;
static dht11_t dht11_data = { -1,-1 };
...
void DHT11_Init(uint8_t dht11_pin)
{
	...
	/* 创建周期获取任务 */
	xTaskCreatePinnedToCore(dht11_task, "dht11_task", 2048, NULL, 5, NULL, APP_CPU_NUM);
}
...
static void dht11_task(void* param)
{
	while (1)
	{
		DHT11_StartGet(&dht11_data.room_temp, &dht11_data.humidity);
		vTaskDelay(pdMS_TO_TICKS(DHT11_GET_PERIOD));
	}
